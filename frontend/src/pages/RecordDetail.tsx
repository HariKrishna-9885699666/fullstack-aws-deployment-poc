...
export function RecordDetail() {
  ...
  const handleFileUpload = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      alert('File type not allowed. Must be PDF, PNG, JPG or TXT.');
      return;
    }
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="bg-slate-800 border-slate-700 text-slate-100 text-sm rounded-lg focus:ring-primary focus:border-primary p-2 outline-none"
          >
          ...
          <Button 
            variant="danger" 
            onClick={() => {
              if (confirm('Delete this record forever?')) deleteMutation.mutate();
            }}
